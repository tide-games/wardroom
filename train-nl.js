...
import { HUNL as HU } from './hunl.js';

setEquityEdges(JSON.parse(fs.readFileSync('buckets-eq.json', 'utf8')));
console.log('HUNL trainer: EHS2 buckets armed, 3-size grid, 100bb');

const ITERS = Number(process.argv[2] || 1_000_000);
// TRAIN_STATE/TRAIN_OUT let parallel workers keep separate files
const STATE_FILE = process.env.TRAIN_STATE || 'train-state-nl.json';
const OUT_FILE = process.env.TRAIN_OUT || 'strategy-nl.json';
...
meta.abstraction = 'hunl-ehs2-3size-v0'; meta.edges = JSON.parse(fs.readFileSync('buckets-eq.json', 'utf8'));
